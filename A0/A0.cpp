...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		ImGui::SliderFloat("rotation", &m_shape_rotation, 0, 2*PI);
		...
		if( ImGui::Button( "Reset" ) ) {
			...
			m_shape_rotation = 0;
			m_shape_size = 1;
			m_shape_translation.x = 0;
			m_shape_translation.y = 0;
...
bool A0::keyInputEvent(int key, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		...
		if (key == GLFW_KEY_UP) {
			m_shape_translation.y += 0.01;
		}
		if (key == GLFW_KEY_DOWN) {
			m_shape_translation.y -= 0.01;
		}	
		if (key == GLFW_KEY_LEFT) {
			m_shape_translation.x -= 0.01;
		}	
